...
func TestClassifierDetectsBotName(t *testing.T) {
	...
	result := classifier.Classify(deltachat.Message{Text: "чатик, сделай саммари", IsGroup: true}, false)
...
func TestClassifierDetectsCorrection(t *testing.T) {
	...
	result := classifier.Classify(deltachat.Message{Text: "ты ошибся, я не разбираюсь в крипте", IsGroup: true}, false)
...
func TestClassifierDetectsEnglishSummaryRequest(t *testing.T) {
	...
	result := classifier.Classify(deltachat.Message{Text: "can someone summarize what we decided", IsGroup: true}, false)
...
func TestClassifierTreatsPrivateChatAsAddressed(t *testing.T) {
	classifier := NewClassifier([]string{"bot"})
	result := classifier.Classify(deltachat.Message{Text: "hello", IsGroup: false}, false)
	if result.Intent != IntentAddressed {
		t.Fatalf("expected addressed intent, got %s", result.Intent)
	}
}
...
func TestClassifierIgnoresBotMessages(t *testing.T) {
	classifier := NewClassifier([]string{"бот"})
	result := classifier.Classify(deltachat.Message{Text: "бот?", IsFromSelf: true}, false)
