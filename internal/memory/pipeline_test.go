...
func TestProcessMessageUpdatesProfileAndTopic(t *testing.T) {
	...
	pipeline := NewPipeline(store, llm.StaticClient{Responses: map[string]json.RawMessage{
		taskUpdateProfile: json.RawMessage(`{"city":"Тбилиси","address":"Tbilisi, Vake","style":"calm","verbosity":"short","expertise":{"go":"medium"},"interests":["llm"]}`),
		taskUpdateTopic:   json.RawMessage(`{"title":"Go и LLM","summary":"Обсуждают Go-бота с LLM","decisions":["писать на Go"],"open_questions":[],"active_participants":["user-1"]}`),
	}}, nil)

	topic, err := pipeline.ProcessMessage(ctx, deltachat.Message{
		ID:       "msg-1",
		ChatID:   "chat-1",
		SenderID: "user-1",
		Sender:   "Алексей",
...
func TestProcessMessageUpdateRebuildsTopicFromEditedText(t *testing.T) {
	...
	pipeline := NewPipeline(store, rebuildClient{}, nil)
	message := deltachat.Message{
		ID:       "msg-1",
		ChatID:   "chat-1",
		SenderID: "user-1",
		Sender:   "Alex",
...
func (rebuildClient) CompleteJSON(_ context.Context, task string, input any, _ string) (json.RawMessage, error) {
	...
	case taskUpdateTopic, taskRebuildTopic:
...
func TestUpdateParticipantLocationFromCoordinates(t *testing.T) {
	...
	pipeline := NewPipeline(store, locationAwareClient{}, fakeMCPTools{})
