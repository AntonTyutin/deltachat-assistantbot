...
// The MCP servers file is treated as trusted configuration; arbitrary public hosts are allowed
// so remote SaaS MCP servers can be used.
...
// LoadMCPServersFromFile reads ASSISTANT_BOT_MCP_SERVERS_FILE. On missing path, unreadable file,
// invalid JSON, or missing mcpServers object it returns nil map and non-fatal warning strings
// for logging. Per-server validation errors skip that server and append a warning.
